...
const NAV_ITEMS: { label: string; href: string; icon: LucideIcon }[] = [
  ...
  { label: "Marks & Reports", href: "/dashboard/parent/marks", icon: BarChart2 },
  { label: "Announcements", href: "/dashboard/parent/announcements", icon: Bell },
  { label: "Performance", href: "/dashboard/parent/performance", icon: TrendingUp },
];
      ...
      <div className="glass rounded-xl p-3 mb-6 border border-white/10">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="text-muted text-xs capitalize">{session?.user?.role ?? "parent"}</p>
          </div>
        </div>
      </div>
            ...
            >
              ...
              >
                <Icon
                  size={17}
                  className={
                    isActive ? "text-electric" : "text-muted group-hover:text-soft-white"
                  }
                />
                {label}
                {isActive && (
                  <ChevronRight size={14} className="ml-auto text-electric" />
                )}
              </Link>
            </motion.div>
          );
